...
const source = 'c:\\조향덕\\교재\\14.인공지능\\문서자동화\\문서7.htm';
const utf8Copy = path.join(__dirname, 'document7-utf8.htm');
const target = path.join(__dirname, '../views/partials/data-analysis-content.ejs');
...
function isMainTitle(inner) {
  return /font-size:12\.0pt/i.test(inner) && /데이터\s*분석/.test(inner);
}
