...
const source = 'c:\\조향덕\\교재\\14.인공지능\\문서자동화\\문서3.htm';
const utf8Copy = path.join(__dirname, 'document3-utf8.htm');
const target = path.join(__dirname, '../views/partials/prompt-template-content.ejs');
...
function isSectionTitle(inner) {
  const text = stripTags(inner);
  return /^\d+\.\s+(\[|[가-힣])/.test(text) && !/^입력 데이터/.test(text);
}

function isMainTitle(inner) {
  return /font-size:12\.0pt/i.test(inner) && /프롬프트 템플릿/.test(inner);
}
