...
const source = 'c:\\조향덕\\교재\\14.인공지능\\HTML\\동영상.htm';
const utf8Copy = path.join(__dirname, 'document9-utf8.htm');
const target = path.join(__dirname, '../views/partials/video-content.ejs');
...
function isMainTitle(inner) {
  return (
    ...
    (/font-size:12\.0pt/i.test(inner) && /동영상|비디오|영상\s*생성/.test(inner))
  );
}
