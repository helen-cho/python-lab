...
const source = 'c:\\조향덕\\교재\\14.인공지능\\문서자동화\\문서8.htm';
const utf8Copy = path.join(__dirname, 'document8-utf8.htm');
const target = path.join(__dirname, '../views/partials/image-generation-content.ejs');
...
function isSectionTitle(inner) {
  ...
  return /^\d+\.\s+[가-힣\[]/.test(text);
}

function isMainTitle(inner) {
  return /font-size:14\.0pt/i.test(inner) || (/font-size:12\.0pt/i.test(inner) && /이미지\s*생성/.test(inner));
}
