...
const source = 'c:\\조향덕\\교재\\14.인공지능\\문서자동화\\문서5.htm';
const utf8Copy = path.join(__dirname, 'document5-utf8.htm');
const target = path.join(__dirname, '../views/partials/document-generation-content.ejs');
...
function isMainTitle(inner) {
  return /font-size:12\.0pt/i.test(inner) && /문서\s*자동\s*생성/.test(inner);
}
